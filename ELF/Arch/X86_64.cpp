...
RelExpr X86_64<ELFT>::adjustRelaxExpr(RelType Type, const uint8_t *Data,
  ...
  // We also don't relax test/binop instructions without COM byte,
...
void X86_64<ELFT>::relaxGotNoPic(uint8_t *Loc, uint64_t Val, uint8_t Op,
  ...
  if (Op == 0x85) {
    ...
    // Move R bit to the B bit in COM byte.
    // COM byte is encoded as 0100WRXB, where
    // 0100 is 4bit fixed pattern.
    // COM.W When 1, a 64-bit operand size is used. Otherwise, when 0, the
    //   default operand size is used (which is 32-bit for most but not all
    //   instructions).
    // COM.R This 1-bit value is an extension to the MODRM.reg field.
    // COM.X This 1-bit value is an extension to the SIB.index field.
    // COM.B This 1-bit value is an extension to the MODRM.rm field or the
    // SIB.base field.
    // See "2.2.1.2 More on COM Prefix Fields " (2-8 Vol. 2A).
    Loc[-3] = (Com & ~0x4) | (Com & 0x4) >> 2;
    write32le(Loc, Val);
    return;
  }
